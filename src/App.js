...
export default function App() {
    ...
    async function handleSignOut() {
        ...
        if (error) {
            ...
            showPopup("Couldn't Sign Out")
        } else {
            // console.log("Signed out successfuly");
            showPopup("Sign Out successful!")
            setUser("");
            localStorage.clear();
        }
    }
                    ...
                    <div className="grid grid-cols-8 w-full h-full p-2">
                        ...
                        <div className="md:col-span-6 md:col-start-3 col-start-1 col-span-8 md:px-8 px-4 py-4 text-zinc-200 overflow-y-scroll scrollbar-thumb-zinc-900 scrollbar-thumb-rounded-2xl scrollbar-track-zinc-900 scrollbar-thin">
                            <Routes location={location} key={location.pathname}>
                                ...
                                <Route path="/blog/*" exact element={<Blogpost showPopup={showPopup} />} />
                                <Route path="/contact_me" exact element={<Contactme showPopup={showPopup} />} />
                                <Route path="/my_work" exact element={<Mywork />} />
                                <Route path="/sign_in" exact element={<Signin showPopup={showPopup} />} />
                                <Route path="/sign_up" exact element={<Signup showPopup={showPopup} />} />
                                <Route path="/write" exact element={<Write showPopup={showPopup} />} />
