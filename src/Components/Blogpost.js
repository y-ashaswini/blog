...
export default function Blogpost({ showPopup }) {
    ...
    async function DEL_BLOG() {
        const { data, error } = await supabase
            ...
            .select();
        if (error) {
            showPopup("Couldn't delete blogpost!")
            console.log(error);
        }
        else {
            showPopup("Blogpost deleted!")
    ...
    async function GET_BLOG(heading) {
        ...
        if (error) {
            console.log(error);
            showPopup("Error fetching blogposts!")
        } else {
    ...
    async function setLike() {
        if (!user || !user.email || (user.email && user.email.trim()) === "") {
            ...
            if (error) {
                console.log(error);
                showPopup("Couldn't like blogpost!")
            }
        }
    }
    ...
    async function setDislike() {
        if (!user || !user.email || (user.email && user.email.trim()) === "") {
            ...
            if (error) {
                console.log(error);
                showPopup("Couldn't dislike blogpost!")
            }
        }
    }
    ...
    async function afterAddComment(temp) {
        ...
        if (error) {
            ...
            console.log(error);
            showPopup("Couldn't add comment!")
        }
    }
